...
This currently supports Kafka 0.8.x and 0.9.x series of releases and MapR Streams.

Configuration for Kafka Source 0.8.x implementation you can find in `documentation for Apache
Flume 1.6.0 release https://flume.apache.org/releases/content/1.6.0/FlumeUserGuide.html#kafka-source`_

Required properties for 0.9.x implementation or MapR Streams are marked in **bold** font.
...
===============================  ===========  ===================================================
**channels**                     --
**type**                         --           The component type name, needs to be ``org.apache.flume.source.kafka.v09.KafkaSource``
**kafka.bootstrap.servers**      --           List of brokers Kafka-Source will connect to, to get the list of topic partitions
                                              This can be a partial list of brokers, but we recommend at least two for HA. This property is
                                              verbose when MapR Streams are used instead of Kafka.
...
**kafka.topics**                 --           Comma-separated list of Kafka or MapR Streams topics we'll read messages from.
                                              For MapR Streams names of topics should be specified like ``/volume_path/stream_name:topic_name``.
...
.. note:: The Kafka Source overrides two Kafka consumer parameters:
          auto.commit.enable is set to "false" by the source and we commit every batch.
          ...
          When all topic names specified in ``kafka.topics`` start with slash ``/`` then Kafka Source switches to MapR Streams mode.


Example for agent named tier1:

.. code-block:: properties for Kafka 0.9

    tier1.sources.source1.type = org.apache.flume.source.kafka.v09.KafkaSource
    tier1.sources.source1.channels = channel1
    tier1.sources.source1.kafka.bootstrap.servers = localhost:9092
    tier1.sources.source1.kafka.topics = test1, test2
    tier1.sources.source1.kafka.consumer.group.id = flume

.. code-block:: properties for MapR Streams

    tier1.sources.source1.type = org.apache.flume.source.kafka.v09.KafkaSource
    tier1.sources.source1.channels = channel1
    tier1.sources.source1.kafka.topics = /vol1/stream1:topic1, /vol1/stream1:topic2
    tier1.sources.source1.kafka.consumer.group.id = flume
...
ssl                          false                                                  Set to true to enable SSL for this AvroSink. When configuring SSL, you can optionally set a "truststore", "truststore-password", "truststore-type", and specify whether to "trust-all-certs". You can also choose to enable "client-auth".
...
client-auth                  false                                                  Set to true to enable SSL Client authentication for this AvroSink. You must set the "keystore", "keystore-password", and "keystore-type" to identify where to find the private key, and its certificate, to be used to identify and authenticate this client.
keystore                     --                                                     The path to a custom Java keystore file. Flume uses the key information in this file to identify and authenticate itself to the remote Avro Source, when you have enabled "client-auth". This must be explicitly specified, for client-auth to be used.
keystore-password            --                                                     The password for the specified keystore, and its private key. A password must be specified if you have set a "keystore".
keystore-type                JKS                                                    The type of the Java keystore. This can be "JKS" or other supported Java keystore type.
...
through various Flume sources. This currently supports Kafka 0.8.x and 0.9.x series of
releases and MapR Streams.

Configuration for Kafka Sink 0.8.x implementation you can find in `documentation for Apache
Flume 1.6.0 release https://flume.apache.org/releases/content/1.6.0/FlumeUserGuide.html#kafka-sink`_
...
Property Name                    Default              Description
===============================  ===================  =============================================================================================
**type**                         --                   Must be set to ``org.apache.flume.sink.kafka.v09.KafkaSink`` for usage of 0.9.x kafka producer
                                                      API and MapR Streams.
...
**kafka.topic**                  default-flume-topic  The topic in Kafka to which the messages will be published. If this parameter is configured,
                                                      messages will be published to this topic. For MapR Streams topic name should be specified like
                                                      ``/volume_path/stream_name:topic_name``.
                                                      If the event header contains a "topic" field, the event will be published to that topic
                                                      overriding the topic configured here.
kafka.flumeBatchSize             100                  How many messages to process in one batch. Larger batches improve throughput while adding latency.
kafka.producer.acks              1                    How many replicas must acknowledge a message before its considered successfully written.
                                                      Accepted values are 0 (Never wait for acknowledgement), 1 (wait for leader only),
...
.. note::   Kafka Sink uses the ``topic`` and ``key`` properties from the FlumeEvent headers to send events to Kafka.
            ...
            When a topic name specified in ``kafka.topics`` starts with slash ``/`` then Kafka Sink switches to MapR Streams mode.
...
.. code-block:: properties for Kafka 0.9
    ...
    a1.sinks.k1.channel = c1
    a1.sinks.k1.type = org.apache.flume.sink.kafka.v09.KafkaSink
...
.. code-block:: properties for MapR Streams
    ...
    a1.sinks.k1.channel = c1
    a1.sinks.k1.type = org.apache.flume.sink.kafka.v09.KafkaSink
    a1.sinks.k1.kafka.topic = /streaming_volume/flume_stream:streams_topic
    a1.sinks.k1.kafka.flumeBatchSize = 20
    a1.sinks.k1.kafka.producer.acks = 1
    a1.sinks.k1.kafka.producer.linger.ms = 1
    a1.sinks.ki.kafka.producer.compression.type = snappy
...
Configuration for Kafka Channel 0.8.x implementation you can find in `documentation for Apache
Flume 1.6.0 release https://flume.apache.org/releases/content/1.6.0/FlumeUserGuide.html#kafka-channel`_
...
Property Name                       Default                           Description
==================================  ==========================  ===============================================================================================================
**type**                            --                          The component type name, needs to be ``org.apache.flume.channel.kafka.v09.KafkaChannel``
**kafka.bootstrap.servers**         --                          List of brokers Kafka-Sink will connect to, to get the list of topic partitions
                                                                This can be a partial list of brokers, but we recommend at least two for HA. This property is
                                                                verbose when MapR Streams are used instead of Kafka.

**kafka.topic**                     flume-channel               Kafka topic which the channel will use. For MapR Streams topic name should be specified like
                                                                ``/volume_path/stream_name:topic_name``.
...
kafka.pollTimeout                   500                         Maximum amount of time in milliseconds channel will wait for events.
...
kafka.consumer.auto.offset.reset    earliest                    When set to ``earliest``, the channel will read all data in the topic, starting from the oldest event
                                                                When ``largest``, it will read only events written after the channel started
Other Kafka Properties              --                          These properties are used to configure the Kafka Producer and Consumer used by the channel.
                                                                ...
                                                                For example: kafka.producer.acks
==================================  ==========================  ===============================================================================================================

.. note:: Kafka Channel does not guarantee that order of events that were put to channel will
          be the same when taking these events.

          When a topic name specified in ``kafka.topic`` starts with slash ``/`` then Kafka Channel switches to MapR Streams mode.

Example for agent named a1:

.. code-block:: properties for Kafka 0.9

    a1.channels.channel1.type   = org.apache.flume.channel.kafka.v09.KafkaChannel
    a1.channels.channel1.capacity = 10000
    a1.channels.channel1.transactionCapacity = 1000
    a1.channels.channel1.kafka.bootstrap.servers=kafka-2:9092,kafka-3:9092
...
.. code-block:: properties for MapR Streams
    ...
    a1.channels.channel1.capacity = 10000
    a1.channels.channel1.transactionCapacity = 1000
    a1.channels.channel1.kafka.topic=/streaming_volume/flume_stream:streams_channel_topic
